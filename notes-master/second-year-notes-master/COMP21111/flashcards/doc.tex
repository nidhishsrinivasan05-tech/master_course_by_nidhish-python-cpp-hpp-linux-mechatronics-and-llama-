...
\flashcard{
  For all interpretation, the formula `$\vee$' evaluates to \blank{0}, and
  `$\wedge$' evaluates to \blank{1}.
}

\card{
  What do you say when an interpretation $I$ if when it is applied to a formula
  $F$, $F \equiv \top$?\\Give two answers.
}{
  We say that $I$ \textit{satisfies} $F$, or that $I$ is a \textit{model} of
  $F$.
}
...
\card{
  ...
  If there is some interpretation that satisfies the formula.
}

\card{
  If a formula is true for every interpretation, then it is...
}{
  Valid (aka a tautology).
}

% TODO: Add re-write rules
